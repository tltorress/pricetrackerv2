...
async def dbCdkeys():
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        ...
        print("it takes Cdkeys: ", time.time() -
...
def get_urls(totalPages):
    ...
    for i in range(1, 426 + 1):
        ...
        if i <= 57:
            urls.append(
                f"https://www.cdkeys.com/playstation-network-psn?locale=all&p=1")
        if i <= 426:
            urls.append(
                f"https://www.cdkeys.com/playstation-network-psn?locale=all&p=1%27")
            pass
    return urls
        ...
        for game in games:
            try:
                ...
                try:
                    ...
                    price = float(game.find(
                        "span", {"class": "special-price"}).find("span", {"class": "price"}).text)
                except:
                    price = float(game.find(
                        "span", {"class": "old-price"}).find("span", {"class": "price"}).text)
...
async def saveGamesCdkeys(allGames):
    ...
    logging.info(
        f'Scraped {len(allGames["ps4"])+len(allGames["xbx"])+len(allGames["pc"])} games of CDkeys')
